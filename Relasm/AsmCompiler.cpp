...
AsmCompiler::AsmCompiler(const QStringList& arguments) : arguments(arguments)
{
	...
	instructionsMethod["mclass"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {CreateMainClass(args, dataStream); return 0; };

	instructionsMethod["new"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		New(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["class"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {CreateClass(args, dataStream); return 0; };

	instructionsMethod["method"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {CreateMethod(args, dataStream); return 0; };

	instructionsMethod["callm"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		CallMethod(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["get"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Get(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["push.str"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int{
		int before = dataStream.device()->size();
		PushStr(args, dataStream);
		return dataStream.device()->size() - before;
	};
	
	instructionsMethod["push.int32"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		PushInt32(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["return"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Return(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["set"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Set(args, dataStream);
		return dataStream.device()->size() - before;
	};
	
	instructionsMethod["local"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Local(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["add"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Add(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["dup"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Dup(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["jmp"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Jmp(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["jmpif"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Jmpif(args, dataStream);
		return dataStream.device()->size() - before;
	};

	instructionsMethod["gc"] = [&](const QStringList& args, QDataStream& dataStream, int& sizeCurrentMethod)->int {
		int before = dataStream.device()->size();
		Gc(args, dataStream);
		return dataStream.device()->size() - before;
	};
...
void AsmCompiler::Start()
{
	...
	ExecuteCode(instructions, output, globalSizeCurrentMethod);
}
...
void AsmCompiler::ExecuteCode(const QList<Instruction>& instructions, QDataStream& dataStream, int& sizeCurrentMethod)
{
	...
	for (int i = 0; i < instructions.size(); ++i)
	{
		...
		{
			sizeCurrentMethod += instructionsMethod[instructionName](args, dataStream, sizeCurrentMethod);
		}
	}
}
...
int AsmCompiler::ProcessSizeInstructions(const QList<Instruction>& instructions)
{
	...
	for (int i = 0; i < instructions.size(); ++i)
	{
		...
		{
			int a = 0;
			sizeMethod += instructionsMethod[instructionName](args, ds, a);
		}
	}
...
void AsmCompiler::CreateMethod(const QStringList& args, QDataStream& dataStream)
{
	...
	ExecuteCode(methodInstructions, dsMethodCode, globalSizeCurrentMethod);
