...
public:
	...
	void ExecuteCode(const QList<Instruction>& instructions, QDataStream& dataStream, int& sizeCurrentMethod);
...
private:
	...
	QFile outputFile;
	QDataStream output;
	QMap<QString, std::function<int (const QStringList&, QDataStream&, int&)>> instructionsMethod;
	QMap<QString, int> tagsCurrentMethod;
	int globalSizeCurrentMethod = 0;
};
